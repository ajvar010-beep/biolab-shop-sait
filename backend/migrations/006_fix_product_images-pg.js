/**
 * PostgreSQL миграция 006: Заменить битые Unsplash-картинки на локальные SVG.
...
module.exports = {
  id: 6,
  name: '006_fix_product_images-pg',
  ...
  async up(db) {
    ...
    for (const [title, slug] of Object.entries(IMAGE_MAP)) {
      ...
      const result = await db.run(
        `UPDATE products
         SET imageUrl = $1, images = $2, updatedAt = $3
         WHERE title = $4 AND imageUrl LIKE '%images.unsplash.com%'`,
    ...
    const rest = await db.run(
      `UPDATE products
       SET imageUrl = '', images = '[]', updatedAt = $1
