...
**Безопасность:** Helmet, Rate Limiting, CORS, XSS Protection, hCaptcha
...
# hCaptcha (защита от ботов на форме заказа)
# Получите бесплатно: https://dashboard.hcaptcha.com/ (Welcome plan)
...
Форма заказа защищена hCaptcha (бесплатная альтернатива reCAPTCHA, работает в РФ):
- Покупатель должен подтвердить, что он не робот
- В девелоп-режиме hCaptcha может быть отключен (HCAPTCHA_SECRET не задан)
- В продакшене без HCAPTCHA_SECRET заказы не пройдут

**Настройка:**
1. Зарегистрируйтесь на https://dashboard.hcaptcha.com/ (Welcome plan — бесплатно)
2. Создайте новый site key
...
HCAPTCHA_SECRET=your-secret-key
```
5. Пересоберите проект (`npm start` или перезапустите на Render)
